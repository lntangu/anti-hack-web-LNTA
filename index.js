...
// BIẾN HỆ THỐNG - GIỮ NGUYÊN NGUYÊN LÝ CŨ
let blockedCount = 0;
let totalRequests = 0;
...
app.use((req, res, next) => {
    // CHẾ ĐỘ TỰ HỦY TẠM THỜI (PANIC MODE)
    if (isPanicMode) {
    ...
    // VÁ LỖI CSP - DẸP BỎ CHỮ ĐỎ TRONG CONSOLE
    ...
    // BẢO MẬT: CHẶN CHAOS ENGINE VÀ SCRIPT LẠ
...
app.post('/api/panic', (req, res) => {
    isPanicMode = true;
    setTimeout(() => { isPanicMode = false; }, 30000); // Hồi sinh sau 30 giây
...
app.get('/api/stats', (req, res) => {
    res.json({
        ...
        cpu: isPanicMode ? "0.0" : (Math.random() * 1 + 5.5).toFixed(1), // Giữ số 5.5%
        ram: isPanicMode ? "0.0" : "12", // Giữ số 12%
        target: "113.191.190.65" // IP mô phỏng kẻ tấn công
    });
});

// GIAO DIỆN NEON CORE V5
app.get('/', (req, res) => {
    res.send(`
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>PHANTOM DEFENSE V5</title>
    <style>
        ...
        .monitor { width: 95%; max-width: 900px; border: 2px solid #0f0; padding: 30px; background: rgba(0, 15, 0, 0.9); box-shadow: 0 0 30px #0f0, inset 0 0 15px #0f0; position: relative; }
        ...
        .panic-btn { grid-column: span 3; background: #300; color: #f00; border: 2px dashed #f00; padding: 15px; cursor: pointer; font-weight: bold; letter-spacing: 5px; margin-top: 15px; }
        .panic-btn:hover { background: #f00; color: #000; box-shadow: 0 0 20px #f00; }
        .panic-active { filter: invert(1) blur(10px); pointer-events: none; }
        h1 { text-align: center; letter-spacing: 10px; text-shadow: 0 0 10px #0f0; margin: 0; }
    </style>
</head>
<body id="b">
    <div class="monitor">
        ...
        <div class="grid">
            ...
            <div class="card"><div style="font-size:10px;">TARGET IP</div><div class="val" id="tip" style="font-size:16px;margin-top:10px;">---</div></div>
    ...
    <script>
        function panic() {
            if(confirm("KÍCH HOẠT TỰ HỦY TẠM THỜI?")) {
...
app.listen(PORT, () => console.log('V5 Online'));
